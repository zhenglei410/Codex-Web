...
- **回答完成的标签提示**：任务结束后浏览器标签闪一次，并把 favicon 保持在提示色（琥珀色）+ 标题加 ✅，
  用户回到该标签页（聚焦/切回可见/点击按键）后自动恢复；
- **回答过程自动贴底**：对话区默认一直跟随到最新输出，用户主动往上翻阅时暂停跟随，
  点「跳到最底部」恢复；新增 `scripts/ui-test.mjs`（jsdom）覆盖这两项行为；
...
- 新增账号弹窗看不到「保存」按钮：弹窗内容区现在自己滚动（`.modal-body` 设 `overflow-y:auto`），
  表单不再被卡片裁掉，保存按钮还会吸附在弹窗底部；模型 / 本机权限弹窗同样受益；
- **新增权限快切**：输入框下方新增「完全控制 / 工作区可写 / 只读」三段快捷选择
  （完全控制 = `danger-full-access`），与顶部下拉双向同步，超出账号权限上限的档位自动禁用，
  底部提示实时显示当前模式；新增 `scripts/browser-test.mjs`（真实 Chrome，14 项）验证布局与快切，
  jsdom 用例扩到 28 项；
- 任务完成后标签/任务栏图标不变色：改为每次都用 canvas 生成 PNG 图标并**替换整个
  `<link rel="icon">` 元素**（只改 href 时 Chrome/Edge 常常不重绘），同时按状态换色：
  空闲=绿色三角、执行中=蓝色、回答结束=琥珀色打勾；
