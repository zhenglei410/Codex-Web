...
| 权限快切 | 输入框下方可直接切换「完全控制 / 工作区可写 / 只读」，完全控制即 `danger-full-access`；超出账号权限上限的档位自动禁用，顶部下拉与快捷选择双向同步 |
| 会话与审计 | 会话归属按账号隔离（成员只看自己的）；每次任务都有审计日志（谁、在哪、用什么模型、结果如何、是否被拦截） |
| 交互细节 | 回答过程中对话区自动贴到最新输出（用户上翻时暂停跟随）；标签/任务栏图标按状态变色（空闲=绿、执行中=蓝、回答完成=琥珀色打勾），完成时闪一次并保留提示色，回到标签页自动恢复 |
